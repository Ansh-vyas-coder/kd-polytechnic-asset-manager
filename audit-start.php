                ...
                <?php foreach ($audits_to_show as $audit): ?>
                    <div class="p-4 space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-2 text-xs">
                            <?php if ($_SESSION['role'] === 'admin'): ?>
                                <div>
                                    <span class="text-gray-400 block">Assigned By:</span>
                                    <span class="font-semibold text-gray-700"><?php echo htmlspecialchars($audit['assigned_by_name'] ?? 'System'); ?></span>
                                </div>
                            <?php endif; ?>
                            <div>
                                <span class="text-gray-400 block">Assigned To:</span>
                                <span class="font-semibold text-gray-700"><?php echo htmlspecialchars($audit['assigned_to_name']); ?></span>
                            </div>
                            <div>
                                <span class="text-gray-400 block">Date Assigned:</span>
                        ...
                        <div class="pt-2 border-t border-gray-50 flex items-center justify-end">
                            <?php if ($_SESSION['role'] === 'admin'): ?>
                                <?php if (isset($audit['status']) && $audit['status'] === 'In Progress'): ?>
                                    <a href="audit_session.php?id=<?php echo $audit['id']; ?>" class="w-full inline-flex items-center justify-center gap-2 rounded-md bg-blue-100 px-3 py-2 text-xs font-semibold text-blue-700 shadow-sm transition hover:bg-blue-200">
                                        View Progress
                                    </a>
                                <?php elseif (isset($audit['status']) && $audit['status'] === 'Completed'): ?>
                                    <a href="audit_results.php?id=<?php echo $audit['id']; ?>" class="w-full inline-flex items-center justify-center gap-2 rounded-md bg-green-100 px-3 py-2 text-xs font-semibold text-green-700 shadow-sm transition hover:bg-green-200">
                                        View Report
                                    </a>
                                <?php else: ?>
                            ...
                            <?php elseif (isset($audit['status']) && $_SESSION['user_name'] === $audit['assigned_to_name'] && $audit['status'] === 'Assigned'): ?>
                                <form action="start_audit.php" method="POST" class="w-full">
                                    <input type="hidden" name="audit_id" value="<?php echo $audit['id']; ?>">
                                    <input type="hidden" name="location_id" value="<?php echo htmlspecialchars($audit['location_id']); ?>">
                                    <button type="submit" class="w-full inline-flex items-center justify-center gap-2 rounded-md bg-green-600 px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-green-700">
                                        <i data-lucide="play" style="width:14px;height:14px"></i>
                                        Start Audit
                                    </button>
                                </form>
            ...
            <div class="block md:hidden divide-y divide-gray-100">
                <?php foreach ($completed_to_show as $audit): ?>
                    <div class="p-4 space-y-2">
                        <div class="flex items-center justify-between gap-2">
                            <span class="font-bold text-gray-900 text-base"><?php echo htmlspecialchars($audit['location_id']); ?></span>
                            <span class="text-xs text-gray-500"><?php echo date('M d, Y', strtotime($audit['audit_date'])); ?></span>
                        </div>
                        <div class="text-xs text-gray-600">
                            <span class="text-gray-400">Audited By:</span> <?php echo htmlspecialchars($audit['full_name'] ?? 'Self'); ?>
                        </div>
                        <div class="pt-2">
                            <a href="audit_results.php?id=<?php echo (int)$audit['id']; ?>" class="w-full inline-flex items-center justify-center gap-2 rounded-md bg-green-100 px-3 py-2 text-xs font-semibold text-green-700 shadow-sm transition hover:bg-green-200">
                                View Report
                            </a>
                        </div>
                    </div>
